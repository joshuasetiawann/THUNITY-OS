/* THUOS - embedded 8x16 bitmap font (the classic VGA text font).
 *
 * Captured once from VGA plane 2 and baked in, so glyphs render in ANY video
 * mode. This matters on real hardware and when a bootloader/UEFI hands the
 * kernel a graphics-mode linear framebuffer (GRUB Multiboot / UEFI GOP): the
 * machine is already out of text mode at hand-off, so the live VGA font cannot
 * be read. With the font embedded, text works everywhere. */
#include "gfx.h"
...
int            gfx_font_ready(void)  { return 1; }

/* The font is embedded, so there is nothing to extract at boot. Kept (as a
 * no-op) so callers/boot log that referenced it still build unchanged. */
void vga_font_extract(void) { /* no-op: font is static, available in any mode */ }
